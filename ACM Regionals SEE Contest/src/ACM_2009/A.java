...
public class A {
	...
	public static void main ( String args[] ) {
		...
		while ( in.hasNext() ) {
			...
			BigDecimal k = new BigDecimal(in.next());
//			System.out.println(n+" "+k);
			BigDecimal four = new BigDecimal("4");
			BigDecimal two = new BigDecimal("2");
			BigDecimal ten_tenth = new BigDecimal("10000000000");
			BigDecimal ten_fifth = new BigDecimal("100000");
			BigDecimal a = ten_tenth.add(four.multiply(n.multiply(k)));
		    BigSquareRoot app = new BigSquareRoot ();
		    app.setTraceFlag (true);
...
//		    System.out.print(a+" ");
		    a = app.get(a);
		    a = a.divide(two.multiply(k),new MathContext(1));  
//		    System.out.println(a+" ");
		    BigInteger min_p = a.subtract(ten_fifth.divide(two.multiply(k),new MathContext(1))).toBigInteger();
		    BigInteger max_p = a.add(ten_fifth.divide(two.multiply(k),new MathContext(1))).toBigInteger();
		    BigInteger one = new BigInteger("1");
		    System.out.println(min_p+" "+max_p);
		    BigInteger p = min_p;
		    BigInteger n_int = n.toBigInteger();
		    BigInteger q;
		    while ( p.compareTo(max_p) < 0 ) {
		    	if ( n_int.mod(p) == BigInteger.ZERO  ) {
		    		q = n_int.divide(p);
