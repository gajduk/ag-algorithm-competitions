package UVA;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Scanner;

class WeightedQuickUnion {

	private int parent_ids[];
	private int weight[];
	private int count;
	private int n;

	public WeightedQuickUnion( int n ) {
		this.parent_ids = new int[n];
		this.weight = new int[n];
		Arrays.fill(this.weight, 1);
		for ( int i = 0 ; i < n ; ++i ) this.parent_ids[i] = i;
		this.n = n;
		this.count = n;
	}
	
	
	public int union(int a, int b) {
		int p = find(a);
		int q = find(b);
		if ( p == q ) return weight[q];
		--count;
		if ( weight[q] > weight[p] ) {
			weight[q] += weight[p];
			parent_ids[p] = q;
			return weight[q];
		}
		else {
			weight[p] += weight[q];
			parent_ids[q] = p;
			return weight[p];
		}
	}

	
	public boolean isConneted(int a, int b) {
		return find(a)==find(b);
	}

	
	public int find(int x) {
		while ( parent_ids[x] != x ) x = parent_ids[x];
		return x;
	}

	
	public int count() {
		return count;
	}
	
	public int[] get_ids() { return parent_ids; }
	
}

public class VirtualFriends {
	
	public static void main(String[] args) {
		Scanner in = new Scanner(System.in);
		int n = in.nextInt();
		for ( int w = 0 ; w < n ; ++w ) {
			HashMap<String,Integer> hash_table = new HashMap<String,Integer>();
			WeightedQuickUnion uf = new WeightedQuickUnion(100000);
			int counter = 0;
			int num_lines = in.nextInt();
			for ( int q = 0 ; q < num_lines ; ++q ) {
				String a = in.next();
				if ( ! hash_table.containsKey(a) ) hash_table.put(a,counter++);
				String b = in.next();
				if ( ! hash_table.containsKey(b) ) hash_table.put(b,counter++);
				System.out.println(uf.union(hash_table.get(a), hash_table.get(b)));
			}
		}
	}

	
}
